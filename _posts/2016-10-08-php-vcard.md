...
{:toc}
   ...
   最近公司有个新的需求，要求将数据库的用户表的内容，转化为vcf格式。这样的话就可以直接导入的通讯录。在github上使用最多的是[v-Card-parser](https://github.com/nuovo/vCard-parser)和[vcard](https://github.com/jeroendesloovere/vcard)，有兴趣的可以研究下；
因为只需要实现导出功能，所以就找了一个最简单的类。代码如下：
```
<?php
/**
 * @file
 ...
 * Time: 11:23
 */
require_once('VCard2.php');

$list = array();
for($i = 0; $i < 50; $i ++){
    $item = array();
    $item['cid'] = rand(10000,1000);
    $item['CustomerName'] = md5(rand(10000,1000000));
    $item['PhoneA'] = "1835551".rand(1000,9999);
    $item['PhoneB'] = "1551".rand(1000,10000);
    $item['CustomerAtt'] = rand(1000,10000);
    $item['CustomerSex'] = "男";
    $item['Type'] = "电话";
    $item['Channel'] = "12121";
    $item['page'] = "12132";
    $item['Keywords'] = "1231";
    $item['firstChoiceSummary'] = "gg";
    $item['QQGroup'] = rand(10000,10000000);
    $list[] = $item;
}

$vcards = export($list);
downloads($vcards,"test_v");
/*
 *$list为数组，应包含字段（cid,CustomerName
,CustomerSex,CustomerAtt,PhoneA,PhoneB,Type,Channel,Page,
Keywords,firstChoiceSummary,QQGroup）
 * */
function export($list){
    ...
    foreach ($list as $item) {
        $vcard = new vCard2;
        $data = array();
        $data['display_name'] = $item['CustomerName'];
        $data['cell_tel'] = $item['PhoneA'];
        $data['home_tel'] = $item['PhoneB'];
        $data['home_city'] = $item['CustomerAtt'];
        $data['note'] = "cid:".$item['cid']."/".
            "性别:".$item['CustomerSex']."/".
            "来源:".$item['Type']."/".
            "渠道:".$item['Channel']."/".
            "来源页面:".$item['page']."/".
            "来源关键字:".$item['Keywords']."/".
            "小结:".$item['firstChoiceSummary']."/".
            "QQ群:".$item['QQGroup'];
        $vcard->set("data",$data);
        $vcards[] = $vcard;
    }
